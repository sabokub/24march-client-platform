...
export default function UpdatePasswordClient({ initialError }: { initialError?: string }) {
  ...
  // Petit confort : afficher direct si le code est mauvais/absent
  if (initialError) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-white flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            ...
            <CardTitle>Impossible de mettre à jour</CardTitle>
            <CardDescription>{initialError}</CardDescription>
          </CardHeader>
          <CardContent>
            <Button asChild className="w-full">
              <Link href="/auth/reset-password">Refaire une demande</Link>
            </Button>
          </CardContent>
        </Card>
      </div>
